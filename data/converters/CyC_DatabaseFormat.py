...
"""
    CyC_DatabaseFormat.py
    This script creates a CyC type Database.
"""
...
from data.types_CyC_TYPES import CyC_DataType, CyC_FilterType
...
def create_data_base_example():
    ...
    db = CyC_DataBase(db_path='C:/dev/src/CyC', core_id=1)
...
class CyC_DataBase:
    ...
    class CyC_Stream:
        def __init__(self, db_path, filter_id, filter_name, input_sources):
            ...
            self.filter_type = CyC_FilterType.CyC_UNDEFINED_FILTER_TYPE
            self.output_type = CyC_DataType.CyC_UNDEFINED
    ...
    def create_db(self):
        ...
        bat.write('python %CyC_DIR%/dojo/tools/Annotation/main_annotation.py %cd%')
        bat.close()
    ...
    def add_data(self, ts_start, ts_stop, data):
        """
            Description:
                Method used for adding data to the created CyberCortex.AI Database
            Parameters:
    ...
    def add_stream(self, filter_type, filter_id, filter_name, input_sources=None):
        ...
        stream = None
        if filter_type in ['CyC_MONO_CAMERA_FILTER_TYPE', 'image', 'camera']:
            stream = CyC_ImageStream(self.db_path, filter_id, filter_name)
        elif filter_type in ['CyC_LIDAR_FILTER_TYPE', 'lidar']:
            stream = CyC_LidarStream(self.db_path, filter_id, filter_name)
        elif filter_type in ['CyC_RADAR_FILTER_TYPE', 'radar']:
            stream = CyC_RadarStream(self.db_path, filter_id, filter_name)
        elif filter_type in ['CyC_VEHICLE_STATE_ESTIMATION_FILTER_TYPE', 'veh_state']:
            stream = CyC_VehStateStream(self.db_path, filter_id, filter_name)
        elif filter_type in ['CyC_OBJECT_DETECTOR_2D_FILTER_TYPE', '2D_obj_det', 'det2d']:
            stream = CyC_2DObjDetStream(self.db_path, filter_id, filter_name, input_sources)
        elif filter_type in ['CyC_OBJECT_DETECTOR_3D_FILTER_TYPE', '3D_obj_det', 'det3d']:
            stream = CyC_3DObjDetStream(self.db_path, filter_id, filter_name, input_sources)
        elif filter_type in ['CyC_IMU_FILTER_TYPE', 'imu']:
            stream = CyC_ImuStream(self.db_path, filter_id, filter_name)
        elif filter_type in ['CyC_ULTRASONICS_FILTER_TYPE', 'ultrasonic']:
            stream = CyC_UltrasonicStream(self.db_path, filter_id, filter_name)
        elif filter_type in ['CyC_LANE_DETECTION_FILTER_TYPE', 'lane', 'lanes']:
            stream = CyC_LaneStream(self.db_path, filter_id, filter_name, input_sources)
        elif filter_type in ['CyC_SEMANTIC_SEGMENTATION_FILTER_TYPE', 'sem_seg', 'semseg', 'semantic']:
            stream = CyC_SemSegStream(self.db_path, filter_id, filter_name, input_sources)
        elif filter_type in ['CyC_RGBDCAMERA_FILTER_TYPE', 'rgbd']:
            stream = CyC_RgbdStream(self.db_path, filter_id, filter_name)
        elif filter_type in ['CyC_ROTARY_ENCODER_FILTER_TYPE', 'wheels_encoder', 'wheels_enc', 'encoder']:
            stream = CyC_WheelsEncoderStream(self.db_path, filter_id, filter_name)
        else:
...
# Different CyC Filter Streams =======================================================================================
class CyC_ImageStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_MONO_CAMERA_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_IMAGE
...
class CyC_LidarStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_LIDAR_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_VOXELS
...
class CyC_RadarStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_RADAR_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_RADAR
...
class CyC_VehStateStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_VEHICLE_STATE_ESTIMATION_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_STATE
...
class CyC_2DObjDetStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_OBJECT_DETECTOR_2D_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_2D_ROIS
...
class CyC_3DObjDetStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_OBJECT_DETECTOR_3D_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_3D_BBOXES
...
class CyC_ImuStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_IMU_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_IMU
...
class CyC_UltrasonicStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_ULTRASONICS_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_ULTRASONICS
...
class CyC_LaneStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_LANE_DETECTION_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_LANES_MODEL
...
class CyC_SemSegStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_SEMANTIC_SEGMENTATION_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_IMAGE
...
class CyC_RgbdStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_RGBDCAMERA_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_IMAGE
...
class CyC_WheelsEncoderStream(CyC_DataBase.CyC_Stream):
    def __init__(self, db_path, filter_id, filter_name, input_sources=()):
        super().__init__(db_path, filter_id, filter_name, input_sources)
        self.filter_type = CyC_FilterType.CyC_ROTARY_ENCODER_FILTER_TYPE
        self.output_type = CyC_DataType.CyC_VECTOR_INT  # TODO check
